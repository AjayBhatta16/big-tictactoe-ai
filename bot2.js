class Bot{
      ...
      for (let i = 0; i < 5; ++i){
        for (let j = 0; j < 5; ++j){
          if (CurrentField.Data[i][j].Type == "Empty"){
            ...
            let Score = this.Estimate(CurrentField, 0, "Human", -Infinity, Infinity, Device);
    ...
    Estimate(GameField, Depth, Player, Alpha, Beta, Device){
      ...
      if ((Depth < 4 && Device == "Laptop") || (Depth < 3 && Device != "Laptop")){
        if (Player == "AI"){
          let BestScore = -Infinity;
          for (let i = 0; i < 5; ++i){
            for (let j = 0; j < 5; ++j){
              if (GameField.Data[i][j].Type == "Empty"){
                ...
                let Score = this.Estimate(GameField, Depth + 1, "Human", Alpha, Beta);
          ...
          for (let i = 0; i < 5; ++i){
            for (let j = 0; j < 5; ++j){
              if (GameField.Data[i][j].Type == "Empty"){
                ...
                let Score = this.Estimate(GameField, Depth + 1, "AI", Alpha, Beta);
                GameField.MakeMove(i + 1, j + 1, "Empty");
